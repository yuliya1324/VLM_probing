...
def _regular_polygon_points(cx: float, cy: float, r: float, n: int, rotation: float = 0) -> list:
...
def _star_points(cx: float, cy: float, r_outer: float, n: int = 5, ratio: float = 0.4) -> list:
...
def draw_shape(draw: ImageDraw.ImageDraw, shape: ShapeInstance) -> None:
    ...
    elif shape.shape_type == ShapeType.OVAL:
        # Wider than tall: width = 2*s, height = 1.3*s
        w = s
        h = s * 0.6
        draw.ellipse([cx - w, cy - h, cx + w, cy + h], fill=fill)

    elif shape.shape_type == ShapeType.RECTANGLE:
        # Wider than tall: width = 2*s, height = 1.2*s
        w = s
        h = s * 0.55
        draw.rectangle([cx - w, cy - h, cx + w, cy + h], fill=fill)


def render_image(
    shapes: list,
    canvas_size: tuple = (448, 448),
